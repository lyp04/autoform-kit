...
public class PrintConfirmationRulesTest {
    ...
    @Test
    public void panelModeCanReportMissingInlineWithoutChangingItsClassification() {
        PrintConfirmationRules.Result result = PrintConfirmationRules.classify(false, false);

        assertEquals(PrintConfirmationRules.Result.MISSING, result);
        assertTrue(PrintConfirmationRules.shouldDeferUntilBatchEnd(result, true));
        assertFalse(PrintConfirmationRules.shouldDeferUntilBatchEnd(result, false));
        assertFalse(PrintConfirmationRules.shouldAlertAfterInline(result));
    }

    @Test
    public void uncertainReprintPostStopsAutomaticRetryAndAlerts() {
        PrintConfirmationRules.Result result = PrintConfirmationRules.classify(
            false, true, true);

        assertEquals(PrintConfirmationRules.Result.UNCERTAIN, result);
        assertTrue(PrintConfirmationRules.shouldAlertAfterInline(result));
        assertFalse(PrintConfirmationRules.shouldDeferUntilBatchEnd(result));
        assertTrue(PrintConfirmationRules.shouldAlertAfterFinalBatchCheck(result));
    }
